...
import { describeWeather, fetchWeather, getLocation, reverseGeocode } from "@/lib/minimaltab/weather";
...
function WeatherWidget() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    getLocation().then(async ({ lat, lon }) => {
  ...
  if (loading) {
    return <div className="flex h-9 items-center gap-1.5 rounded-full border border-border bg-card/70 px-3 text-xs text-muted-foreground backdrop-blur">
  ...
  return <div className="group relative flex h-9 items-center gap-2 rounded-full border border-border bg-card/70 px-3 text-xs backdrop-blur transition-colors hover:bg-card">
    ...
    /* Mini hourly tooltip */
      ...
      <div className="pointer-events-none absolute right-0 top-full z-20 mt-2 w-44 rounded-xl border border-border bg-popover p-2 opacity-0 shadow-lg transition-opacity group-hover:opacity-100">
        ...
        <div className="flex justify-between">
          {weather.hourly.map((h) => <div key={h.time} className="flex flex-col items-center gap-1">
