...
function Hero() {
  const [name] = useLocalStorage("mt.name", "");
  const [dailyFocus] = useLocalStorage("mt.dailyFocus", "");
  ...
  /> : " "}
      ...
      {dailyFocus ? <p className="mx-auto mt-5 max-w-xl rounded-full border border-border bg-card/60 px-4 py-1.5 text-sm text-foreground/80 backdrop-blur">
          {dailyFocus}
        </p> : null}
