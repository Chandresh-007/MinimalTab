...
function Settings({
  ...
  if (!open) return null;
  return <div className="fixed inset-0 z-40" role="dialog" aria-modal="true" aria-label="Settings">
  ...
  >
    ...
    className="w-full rounded-lg border border-border bg-transparent px-3 py-2 outline-none focus:border-foreground/40"
  >
              {Object.values(ENGINES).map((e) => <option key={e.id} value={e.id}>{e.name}</option>)}
            </select>
          </section>
