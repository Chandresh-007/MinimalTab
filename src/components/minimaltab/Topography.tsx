...
export function Topography(props: TopographyProps) {
  ...
  useEffect(() => {
    ...
    const syncProps = () => {
      ...
      u.uMorphAmount.value = p.morphAmount ?? 3.0;
      u.uMorphSpeed.value = p.morphSpeed ?? 0.05;
      u.uBands.value = p.bands ?? 2.0;
      u.uThickness.value = p.thickness ?? 0.01;
      u.uScale.value = p.scale ?? 1.0;
      u.uPixelSize.value = p.pixelSize ?? 1.0;
      u.uGlow.value = p.glow ?? 0.5;
      u.uColorMode.value = colorModeToFloat(p.colorMode ?? "elevation");
      u.uContrast.value = p.contrast ?? 3.0;
      u.uBrightness.value = p.brightness ?? 1.0;
      u.uFillBands.value = p.fillBands ? 1.0 : 0.0;
      u.uOpacity.value = p.opacity ?? 1.0;
      u.uGrain.value = (p.grain ?? true) ? 1.0 : 0.0;
      u.uGrainIntensity.value = p.grainIntensity ?? 0.05;
      u.uLow.value = new Float32Array(hexToRgb(p.lowColor ?? "#5227FF"));
      u.uMid.value = new Float32Array(hexToRgb(p.midColor ?? "#FF9FFC"));
      u.uHigh.value = new Float32Array(hexToRgb(p.highColor ?? "#FFFFFF"));
      u.uMouseEnabled.value = (p.mouseInteraction ?? true) ? 1.0 : 0.0;
      u.uMouseRadius.value = p.mouseRadius ?? 0.3;
      u.uMouseStrength.value = p.mouseStrength ?? 0.4;
    };
    syncProps();
    ...
    const loop = (t: number) => {
      ...
      const interval = reduced.matches ? 1000 / 15 : 1000 / 60;
      ...
      const sp = (u.uSpeed.value as number) * (reduced.matches ? 0.3 : 1);
