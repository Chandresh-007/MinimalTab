...
`;
const ctxMap = /* @__PURE__ */ new WeakMap();
const GradientWaves = ({
  horizonColor = "#5227FF",
  waveColor = "#FF9FFC",
  crestColor = "#FFFFFF",
  ...
  useEffect(() => {
    ...
    try {
      ...
      const minFrameMs = reduced ? 1e3 : 0;
      let lastFrame = 0;
      let frames = 0;
      ...
      const loop = (t) => {
        ...
        lastFrame = t;
        program.uniforms.iTime.value = (t - t0) * 1e-3 * (reduced ? 0.15 : 1);
        ...
        if (t - statusAt >= 1e3) {
          statusRef.current?.({
            running: true,
            fps: Math.round(frames * 1e3 / (t - statusAt))
          });
          frames = 0;
          statusAt = t;
        }
...
var stdin_default = GradientWaves;
export {
