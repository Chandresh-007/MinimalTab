...
function CommandPalette({
  ...
  const filtered = q ? staticItems.filter((i) => i.label.toLowerCase().includes(q)) : staticItems;
  const showSearchItem = q.length > 0;
  const searchItem = showSearchItem ? {
    id: "search",
    label: `Search "${query}"`,
    icon: Search,
    group: "Search",
  ...
  const items = searchItem ? [searchItem, ...filtered] : filtered;
  ...
  />
            ...
            <ul className="max-h-[50vh] overflow-y-auto p-2">
              {items.length === 0 && <li className="px-3 py-6 text-center text-sm text-muted-foreground">No results</li>}
              {items.map((it, i) => {
    const Icon = it.icon;
    ...
    return <li key={it.id}>
      ...
      className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm transition-colors ${isActive ? "bg-muted" : "hover:bg-muted/60"}`}
    >
