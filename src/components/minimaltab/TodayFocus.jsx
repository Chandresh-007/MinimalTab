...
function TodayFocus({ triggerStart }) {
  const [goal, setGoal] = useLocalStorage("mt.goal", "");
  const [tasks, setTasks] = useLocalStorage("mt.tasks", []);
  const [streak] = useLocalStorage("mt.streak", 1);
  ...
  return <section aria-labelledby="today-heading" className="rounded-2xl border border-border bg-card/70 p-6 backdrop-blur">
    ...
    return <button
      ...
      className={`relative flex flex-1 items-center justify-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${active ? "text-background" : "text-muted-foreground hover:text-foreground"}`}
    >
              {active && <motion.span
  ...
  />
      ...
      <div className="mt-3 space-y-1">
        {tasks.map((t) => <div key={t.id} className="group flex items-center gap-2 rounded-md px-1 py-1">
            <button
    onClick={() => toggle(t.id)}
