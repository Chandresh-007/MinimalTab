...
import { DEFAULT_LINKS, ICONS } from "@/lib/minimaltab/links";
...
function QuickAccess() {
  const [links, setLinks] = useLocalStorage("mt.quicklinks", DEFAULT_LINKS);
  const [topSites, setTopSites] = useLocalStorage(TOP_SITES_KEY, {});
  ...
  const sortedTopSites = Object.entries(topSites).sort((a, b) => b[1] - a[1]).slice(0, 4).map(([url2]) => {
    const link = links.find((l) => l.url === url2);
    return link ? { name: link.name, url: url2 } : { name: new URL(url2).hostname.replace(/^www\./, ""), url: url2 };
  });
  ...
  const startPointerDrag = (e, id, label) => {
  ...
  return <section aria-labelledby="quick-access-heading">
      ...
      {sortedTopSites.length > 0 && <div className="mb-3 flex flex-wrap items-center gap-2">
          <span className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-muted-foreground">
            <Clock className="h-3 w-3" /> Top sites
          </span>
          {sortedTopSites.map((site) => <a
    key={site.url}
    href={site.url}
    target="_blank"
    rel="noreferrer"
    onClick={() => trackClick({ url: site.url })}
  ...
  >
      ...
      {importOpen && <div className="mb-3 rounded-xl border border-border bg-card p-3">
  ...
  />
      ...
      {adding && <div className="mb-3 flex flex-wrap items-center gap-2 rounded-xl border border-border bg-card p-2">
          <input
    value={name}
  ...
  >
    ...
    >
      ...
      className={`flex items-center gap-2 rounded-2xl border border-border bg-card px-2.5 py-3 transition-all duration-150 hover:-translate-y-0.5 hover:shadow-[0_1px_2px_rgba(0,0,0,0.04),0_8px_24px_-12px_rgba(0,0,0,0.12)] ${isOver ? "border-foreground/40 ring-2 ring-foreground/10" : ""}`}
    >
