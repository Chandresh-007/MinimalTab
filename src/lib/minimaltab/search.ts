...
// Prefix -> engine id (or custom handler)
...
export function routeSearch(input: string, defaultEngine: string): { url: string; engine: string; query: string } {
  ...
  if (PREFIX_MAP[prefix] && rest.length) {
    const engine = ENGINES[PREFIX_MAP[prefix]];
    const q = rest.join(" ");
    return { url: engine.url(q), engine: engine.name, query: q };
  }
